...
{
    /// <summary>
    /// Update the selected template for exporting task to Excel document.
    /// </summary>
    public class UpdateTemplateCommand : BaseCommandDisableAbleAsync<TaskExportTemplateDto>
    {
        ...
        public override Task ExecuteAsync(TaskExportTemplateDto template)
        {
            ...
            validator.ValidateAndThrow(template, $"default,{nameof(TaskExportTemplateDto.Id)}");
            ...
            using (var transaction = repository.GetTransaction())
            {
                ...
                repository.Update(templateEntity);
